const HSThemeAppearance = {
  init() {
    ...
    document.querySelector("html").classList.contains("dark") || this.setAppearance(e);
  },
  _resetStylesOnLoad() {
  ...
  setAppearance(e, t = !0, a = !0) {
    ...
    t && localStorage.setItem("hs_theme", e),
      ...
      setTimeout(() => { r.remove() }),
      a && window.dispatchEvent(new CustomEvent("on-hs-appearance-change", { detail: e }))
  },
  getAppearance() {
    let e = this.getOriginalAppearance();
    return e = "auto" === e ? window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "default" : e
  },
  getOriginalAppearance() {
    return localStorage.getItem("hs_theme") || "dark"
  }
},
  venobox = (HSThemeAppearance.init(),
    ...
    document.querySelectorAll(".js-clipboard").forEach(c => {
      ...
      new ClipboardJS(c, {
        text: e => {
          ...
          return t || (t = e.dataset.clipboardTarget,
            "SELECT" === (e = document.querySelector(t)).tagName || "INPUT" === e.tagName || "TEXTAREA" === e.tagName ? e.value : e.textContent)
        }
      }).on("success", () => {
